...
/* Obtain number of tasks and task ID */
    ...
    /* allocate buffer for timing send, recv */
    double* sendtimes = (double*) malloc(ROUNDS*sizeof(double));
    double sendtotaltime = 0.0;
    double sendavg = 0.0;
    double sendstddev = 0.0;
    double* recvtimes = (double*) malloc(ROUNDS*sizeof(double));
    double recvtotaltime = 0.0;
    double recvavg = 0.0;
    double recvstddev = 0.0;
    double start, end;
	...
	if (taskid != MASTER) {
	    ...
	    sendtimes[i] = end - start;
	    sendtotaltime += end - start;
	} 
	else
	{
		...
		recvtimes[i] = end - start;
		recvtotaltime += end - start;
	...
	sendavg = sendtotaltime/((double) ROUNDS);
	double sum = 0.0;
	for (i = 0; i < ROUNDS; i++) 
	sum += (sendtimes[i] - sendavg)*(sendtimes[i] - sendavg);
    
	sum = sum / ((double) ROUNDS);
	sendstddev = sqrt(sum);
	fprintf(stdout, "mpi_pi_send [%d]: send N: %d\n", taskid, ROUNDS);
	fprintf(stdout, "mpi_pi_send [%d]: send totaltime: %.10e\n", taskid, sendtotaltime);
	fprintf(stdout, "mpi_pi_send [%d]: send avg: %.10e\n", taskid, sendavg);
	fprintf(stdout, "mpi_pi_send [%d]: send std-dev: %.10e\n", taskid, sendstddev);

    } else {
	/* compute standard deviation */
	recvavg = recvtotaltime/((double) ROUNDS);
	double sum = 0.0;
	for (i = 0; i < ROUNDS; i++) 
	sum += (recvtimes[i] - recvavg)*(recvtimes[i] - recvavg);
    
	sum = sum / ((double) ROUNDS);
	recvstddev = sqrt(sum);
	fprintf(stdout, "mpi_pi_recv [%d]: recv N: %d\n", taskid, ROUNDS);
	fprintf(stdout, "mpi_pi_recv [%d]: recv totaltime: %.10e\n", taskid, recvtotaltime);
	fprintf(stdout, "mpi_pi_recv [%d]: recv avg: %.10e\n", taskid, recvavg);
	fprintf(stdout, "mpi_pi_recv [%d]: recv std-dev: %.10e\n", taskid, recvstddev);
    }
    free(sendtimes);
    free(recvtimes);
