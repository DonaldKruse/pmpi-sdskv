...
int main (int argc, char *argv[])
{
    ...
    //printf ("mpi_pi_reduce: MPI task %d has started...\n", taskid);
    
    /* Set seed for random number generator equal to task ID */
    srandom (taskid);
    
    /* allocate buffer for timing reduce */
    double* reducetimes = (double*) malloc(ROUNDS*sizeof(double));
    double reducetotaltime = 0.0;
    double reduceavg = 0.0;
    double reducestddev = 0.0;
    double start, end;
    ...
    if (taskid == MASTER) 
        //printf ("\nmpi_pi_reduce: Real value of PI: 3.1415926535897 \n");
    
    
    //printf("mpi_pi_reduce [%d]: %.10e seconds in MPI_Reduce. Called %d times.\n", taskid, reducetotaltime, ROUNDS);

    /* compute standard deviation */
    reduceavg = reducetotaltime/((double) ROUNDS);
    double sum = 0.0;
    for (i = 0; i < ROUNDS; i++) 
	sum += (reducetimes[i] - reduceavg)*(reducetimes[i] - reduceavg);
    
    sum = sum / ((double) ROUNDS);
    reducestddev = sqrt(sum);
    fprintf(stdout, "mpi_pi_reduce [%d]: reduce N: %d\n", taskid, ROUNDS);
    fprintf(stdout, "mpi_pi_reduce [%d]: reduce totaltime: %.10e\n", taskid, reducetotaltime);
    fprintf(stdout, "mpi_pi_reduce [%d]: reduce avg: %.10e\n", taskid, reduceavg);
    fprintf(stdout, "mpi_pi_reduce [%d]: reduce std-dev: %.10e\n", taskid, reducestddev);
